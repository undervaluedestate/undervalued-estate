...
const router = Router();
router.get('/', requireApiSecret(), async (_req, res) => {
    try {
...
router.post('/schedule/log', requireApiSecret(), async (req, res) => {
    try {
        const supa = getAdminClient();
        const payload = {
...
router.get('/schedule/last', requireApiSecret(), async (req, res) => {
    try {
        const supa = getAdminClient();
...
router.get('/schedule/last/paginated', requireApiSecret(), async (req, res) => {
    try {
        const supa = getAdminClient();
        const limit = Math.min(100, Math.max(1, Number(req.query?.limit ?? 20)));
