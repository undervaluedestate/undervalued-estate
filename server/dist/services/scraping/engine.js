...
export class ScrapeEngine {
    ...
    limit;
    concurrency = 5;
    constructor({ adapters = [], concurrency = 5 } = {}) {
    ...
    async run({ adapterName, maxPages = 1, maxUrls = 10, requestTimeoutMs = 12000, discoveryTimeoutMs = 5000, extraStartUrls = [], extraListingType, concurrency } = {}) {
        const admin = getAdminClient();
        const adapters = adapterName ? [this.getAdapterByName(adapterName)].filter(Boolean) : this.adapters;
        if (!adapters.length) {
        ...
        for (const adapter of adapters) {
            ...
            const ctx = {
                http: { getText },
                cheerio,
                log: (...args) => console.log(`[${meta.name}]`, ...args),
                adminClient: admin,
                source,
                maxPages,
                requestTimeoutMs,
            ...
            for (const u of urls) {
                try {
                    ...
                    if (!canonicalToUrl.has(c))
                        canonicalToUrl.set(c, u);
                }
                catch {
                    if (!canonicalToUrl.has(u))
                        canonicalToUrl.set(u, u);
                }
            }
            ...
            try {
                ...
                if (ids.length) {
                    ...
                    if (keepUrls.size > 0)
                        uniqueUrls = uniqueUrls.filter((u) => keepUrls.has(u));
                }
            }
            catch { /* best-effort */ }
            // Utility sleep
            ...
            const tasks = uniqueUrls.map((url) => runLimit(async () => {
                ...
                for (let attempt = 1; attempt <= maxAttempts; attempt++) {
                    try {
                        ...
                        if (!rawItem || !rawItem.external_id)
                            return 0;
                        const payload = normalizeToProperty({ ...rawItem, source });
                        if (ctx.extra?.listingType)
                            payload.listing_type = ctx.extra.listingType;
