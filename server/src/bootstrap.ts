...
function parseList(envVal?: string | null): string[] {
  if (!envVal) return [];
...
export async function runBootScrape(): Promise<void> {
  try {
    ...
    for (const name of adaptersToRun) {
      ...
      const seeds = parseList((process.env as any)[seedsEnvKey]);
      ...
      try {
        const results = await engine.run({
          ...
          extraListingType: listingType as any,
          concurrency,
        });
