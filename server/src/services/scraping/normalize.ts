...
function mapPropertyType(t: any): PropertyType {
  ...
  if (s.includes('terraced') || /end\s*of\s*terrace/.test(s) || /mid\s*terrace/.test(s) || /\bterrace\b/.test(s)) return 'terraced_house';
  if (s.includes('townhouse') || s.includes('terrace')) return 'townhouse';
