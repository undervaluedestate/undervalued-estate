...
function buildQueryForAlert(supa: any, alert: any) {
  ...
  if (alert.neighborhood) q = q.eq('neighborhood', alert.neighborhood);
  if (alert.property_type) q = q.eq('property_type', alert.property_type);
  if (alert.min_price != null) q = q.gte('price', alert.min_price);
  if (alert.max_price != null) q = q.lte('price', alert.max_price);
  if (alert.min_size_sqm != null) q = q.gte('size_sqm', alert.min_size_sqm);
  if (alert.max_size_sqm != null) q = q.lte('size_sqm', alert.max_size_sqm);
  const threshold = Number(alert.threshold_percent) || 0;
  if (threshold > 0) q = q.lte('pct_vs_market', -Math.abs(threshold));
  q = q.order('scraped_at', { ascending: false }).limit(200);
  return q;
}

export async function dispatchAlerts({ maxPerAlert = 20 }: { maxPerAlert?: number } = {}) {
